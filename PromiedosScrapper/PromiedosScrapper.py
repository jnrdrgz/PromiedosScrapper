...
class PromiedosScrapper:
	...
	def get_matches(self, league, lround, json=False):
		...
		for x in s.findAll(True, {'class':['datoequipo', 'falta', 'finaliza', 'horariopartido', 'forma']}):
			#return x
			if not x.attrs['class'][0] == "forma":
				todo.append(x.get_text())
			else:
				todo.append(x.findNext().attrs["href"].replace("ficha.php?id=",""))
